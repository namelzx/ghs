    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column label="广告图片" width="120" align="center">
          <template slot-scope="scope"><img style="height: 80px" :src="scope.row.images_url"></template>
        </el-table-column>
        <el-table-column label="时间" width="220" align="center">
          <template slot-scope="scope">
            <p>开始时间：{{ scope.row.startTime | parseTime }}</p>
            <p>到期时间：{{ scope.row.endTime | parseTime }}</p>
          </template>
        </el-table-column>
...
<script>
import { fetchList, updateStatus, GetIdByDel } from '@/api/banner'
...
export default {
  ...
  methods: {
    ...
    handleUpdate(index, row) {
      this.$router.push({ path: '/sms/advertise/update/' + row.id })
    },
    getList() {
