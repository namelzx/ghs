      ...
      <el-table
        ...
        <el-table-column label="客户信息" align="left" fixed min-width="150px">
          <template slot-scope="scope">
            <div>
              <div style="margin-left: 10px" class="link-type" v-if="scope.row.user" @click="handleUpdate(scope.$index,scope.row.id)">{{
        ...
        <el-table-column label="操作" fixed="right" align="center" width="120px" class-name="small-padding">
          ...
          <!--<div v-if="scope.row.status===2">-->
            <el-button type="text" size="mini"
                       @click="handleUpdate(scope.row.id,1)">通过
            </el-button>
            <el-button v-waves :loading="scope.row.delete" type="text" size="mini"
                       @click="handleDelete(scope.row.id,3)">驳回
            </el-button>
          <!--</div>-->
          </template>
        </el-table-column>
...
<script>
  import { GetDataByList, PostDataByAudit, GetIdByDel,PostRoleByUpdate } from '@/api/shopwithdrawal'
