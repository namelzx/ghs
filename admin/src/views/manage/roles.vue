  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="状态" width="110px" align="center">
        <template slot-scope="scope">
          <span
            :class="{'el-icon-success text-green':scope.row.status == 1,'el-icon-error text-red':scope.row.status == 0}"
            @click="handleModifyStatus(scope.$index,scope.row.id,scope.row.status)">{{ scope.row.status | statusFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" min-width="80px" class-name="small-padding">
        <template slot-scope="scope">
          <el-tooltip content="编辑" placement="top">
            <el-button v-waves type="primary" size="mini" @click="handleUpdate(scope.$index,scope.row.id)">
              编辑
            </el-button>
          </el-tooltip>
          <el-tooltip content="删除" placement="top">
            ...
            <el-button :loading="scope.row.delete" type="danger" size="mini"
    ...
    <permissionsForm ref="fromRoles" @updateRow="updateRow"/>
...
<script>
  // import { getList, del, change, delAll, changeAll ,getListorlse} from '@/api/admin'
var self;
  // import { getList, del, change, delAll, changeAll } from '@/api/admin'

  import { change, changeAll, del, delAll, getList } from '@/api/ol'
  import waves from '@/directive/waves'
  import { getArrByKey, pickerOptions } from '@/utils'
  import rolesForm from './roles/form'

  import permissionsForm from './roles/permissions'

  export default {
    name: 'Roles',
    components: { rolesForm, permissionsForm },
    directives: {
      waves
    },
    filters: {
    ...
    methods: {
      ...
      fetchList() {
        ...
        // getListorlse(this.listQuery).then(response => {
        //   self.list = response.data.data
        //   self.total = response.data.total
        //   self.listLoading = false
        // })
