...
Page({
  ...
  creimg: function () {
    ...
    setTimeout(function () {
      ...
      //将方形图片处理成圆形头像
      var avatarurl_heigth = parseInt(myCanvasHeight / 10);   //绘制的头像高度
      var avatarurl_width = avatarurl_heigth
      var avatarurl_x = myCanvasWidth - (myCanvasWidth-20);   //绘制的头像在画布上的位置
      var avatarurl_y = myCanvasHeight - (myCanvasHeight*1) //绘制的头像在画布上的位置
      console.log(myCanvasHeight,'3')
      that.setData({
      ...
      context.drawImage('../../imgs/goodsbg2.jpg', 0, 0, myCanvasWidth, myCanvasHeight)
      context.fill()
      context.drawImage(that.data.images_url, myCanvasWidth - (myCanvasWidth * 0.95), myCanvasHeight - (myCanvasHeight*0.85), myCanvasWidth/1.1 , myCanvasHeight/2.5 )
      context.fill()
    ...
    //价格模块
      ...
      context.setFontSize(14)
      context.fillText(that.data.userinfo.nickName + "正在分享商品", myCanvasWidth - (myCanvasWidth * 0.82), myCanvasHeight - (myCanvasHeight * 0.95))
     
    
      context.drawImage(that.data.code, myCanvasWidth - (myCanvasWidth*0.9), myCanvasHeight - (myCanvasHeight*0.22), myCanvasWidth / 4, myCanvasWidth/4); 
      context.draw()
