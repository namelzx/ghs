    ...
    public function PostDataByUpdate()
    {
        ...
        if (!empty($getorder['shop_id'])) {
            ...
            if ((int)$data['status'] === 2) {
                ...
                $semd = [
                    'template_id' => 'gHQSOcCng-4XY0DlM2b7dZlifhDeu24qoKiAcfbFa5s', // 所需下发的订阅模板id
                    'touser' => 'oD-YF5vPsvPV1mcZ1Eo3VaexBCww',     // 接收者（用户）的 openid
                ...
                $we->SendMessage($semd);
                if (!empty($getorder['dis_id'])) {

                    $openid = UserModel::where('id', $getorder['dis_id'])->value('openid');
                    $semd = [
                        'template_id' => 'gHQSOcCng-4XY0DlM2b7dZlifhDeu24qoKiAcfbFa5s', // 所需下发的订阅模板id
                        ...
                        'data' => [         // 模板内容，格式形如 { "key1": { "value": any }, "key2": { "value": any } }
                            ...
                            'name5' => [
                                'value' => $getorder['buyerName'] ? $getorder['buyerName'] : '你邀请的好友下单~',
                            ],
                            'thing6' => [
                                'value' => $getorder['addressText'],
                            ],
...
//
        ...
        if (!empty($data['status']) && (int)$data['status'] === 2) {
            $goods = OrderGoodsModel::where('order_id', $data['id'])->select();
