...
//            }
        ...
        $user = my_model('OrderModel', 'model', 'admin')->GetDataByList($data, $where);

        $coudata = Db::name('order')->field('count(id) as total,status')->group('status')->select();
        $temp = [
